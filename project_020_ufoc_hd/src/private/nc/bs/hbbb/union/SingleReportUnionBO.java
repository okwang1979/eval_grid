...
//				RepDataVO[] repData = RepDataBO_Client.loadRepData(pk_report, null, vo, null);
	...
	private void doDynRepUnion(String pk_report,HBUnionVouchDataCond vouchDataCond,
		...
		// @edit by zhoushuang at 2015-11-4,上午9:12:46 动态表合并效率优化 
 		// -------------start-------------
		//<关键字pk,keyword?>
//		Map<String, String> pkToPos = new HashMap<String, String>();
//		for (int j = 0; j < mainKeys.length; j++) {
//			pkToPos.put(mainKeys[j].getPk_keyword(), "keyword"+hashPos.get(j)+1);
//		}
		
		//按版本分组，个别表一组，合并表一组 <版本，单位列表>，除单位关键字，其他的应该是一样的
		Map<Integer, List<String>> verToOrgsMap = new HashMap<Integer, List<String>>();
		
		for(MeasurePubDataVO vo : currPubDatalist){
			for (int j = 0; j < mainKeys.length; j++) {
				String value = vo.getKeywordByPK(mainKeys[j].getPk_keyword());
				//单位
				if (mainKeys[j].getPk_keyword().equals(KeyVO.CORP_PK)) {
					//1、清理所有虚单位的动态表个别报表,因为虚单位的个别报表数据是不能参与合并的
					if (lstEntityOrgs == null || lstEntityOrgs.size() == 0 || !lstEntityOrgs.contains(value)) {
						if(vo.getVer() == 0 || (vo.getVer()+"").startsWith(IDataVersionConsts.VERTYPE_SEPERATE_ADJUST+"")){//个别报表或个别报表调整表取个别报表版本
							if (!verToOrgsMap.containsKey(0)) {
								List<String> sepOrgs = new ArrayList<String>();
								verToOrgsMap.put(0, sepOrgs);
							}
							verToOrgsMap.get(0).add(value);
						}else if((vo.getVer()+"").startsWith(IDataVersionConsts.VERTYPE_HBBB+"")||(vo.getVer()+"").startsWith(IDataVersionConsts.VERTYPE_HBBB_ADJUST+"")){//合并报表或合并报表调整表取合并报表版本
							if (!verToOrgsMap.containsKey(mainPubData.getVer())) {
								List<String> sepOrgs = new ArrayList<String>();
								verToOrgsMap.put(mainPubData.getVer(), sepOrgs);
							}
							verToOrgsMap.get(mainPubData.getVer()).add(value);
						}
					}
				}				
			}
		...
		//其他关键字
		Map<String, String> mainKeyToValueMap = new HashMap<String, String>();
		MeasurePubDataVO dataVO = currPubDatalist.get(0);
		for (int j = 0; j < mainKeys.length; j++) {
			if (!mainKeys[j].getPk_keyword().equals(KeyVO.CORP_PK)){
				String value = dataVO.getKeywordByPK(mainKeys[j].getPk_keyword());
				mainKeyToValueMap.put(mainKeys[j].getPk_keyword(), value);
			}
		}
...
//		取得所有参与合并的组织的动态区关键字MeasurePubDataVO
		List<MeasurePubDataVO> allDynMeasurePubDataLst = new ArrayList<MeasurePubDataVO>();
		for (Integer ver : verToOrgsMap.keySet()) {
			StringBuilder whereSQL = new StringBuilder();
			List<String> orgList = verToOrgsMap.get(ver);
			if (orgList != null && orgList.size() > 0 ) {
				for (int j = 0; j < mainKeys.length; j++) {
					if (mainKeys[j].getPk_keyword().equals(KeyVO.CORP_PK)) {
						whereSQL.append(UFOCSqlUtil.buildInSql(" keyword"+(hashPos.get(j) + 1), orgList));
					} else {
						whereSQL.append(" keyword");
						whereSQL.append(hashPos.get(j) + 1);
						whereSQL.append("='");
						whereSQL.append(mainKeyToValueMap.get(mainKeys[j].getPk_keyword()));
						whereSQL.append("'");
					}
					whereSQL.append(" and  ");
				}
				whereSQL.append(" ver= " + ver );
			}
			MeasurePubDataVO[] allDynPubDatas = MeasurePubDataBO_Client.findBySqlCondition(pk_subgroup, whereSQL.toString());
			allDynMeasurePubDataLst.addAll(Arrays.asList(allDynPubDatas));
		}
...
//		}
		...
		if(dynMapRepdatavos != null){
			...
			Map<String, Map<String, UFDouble>> resultMap = getMapofTotalValue(dynMeasures, subGropVO, pk_dynkeyword,verToOrgsMap,mainKeys,hashPos,mainKeyToValueMap);
...
//		}
		...
		if (map.size() > 0) {
			...
			// 动态表，固定区先进行合并，再做计算--保利需求
			String aloneid = mainPubData.getAloneID();
			...
			MeasureDataBO_Client.editRepData(aloneid, datavos);
		}
...
//	}
	 ...
	 * 动态表合并效率优化 
	 * 
	 * @edit by zhoushuang at 2015-11-4,上午9:12:46 
	 */
	@SuppressWarnings("unchecked")
	private Map<String, Map<String, UFDouble>> getMapofTotalValue(
			Set<MeasureVO> dynMeasures, KeyGroupVO subGropVO,
			String pk_dynkeyword, Map<Integer, List<String>> verToOrgsMap,
			KeyVO[] mainKeys, Hashtable<Integer, Integer> hashPos,
			Map<String, String> mainKeyToValueMap) throws BusinessException,
			DAOException {
		...
		for (Map.Entry<String, List<MeasureVO>> enter : mapOfMeasure.entrySet()) {
			...
			// JIAAH 更改sql
			// selectSql.append(UFOCSqlUtil.buildInSql("t2.alone_id",
			// aloneids));
			
			for (Integer ver : verToOrgsMap.keySet()) {
				StringBuilder sqlString = new StringBuilder(selectSql);
				List<String> orgList = verToOrgsMap.get(ver);
				if (orgList != null && orgList.size() > 0) {
					for (int j = 0; j < mainKeys.length; j++) {
						if (mainKeys[j].getPk_keyword().equals(KeyVO.CORP_PK)) {
							sqlString.append(UFOCSqlUtil.buildInSql(" keyword" + (hashPos.get(j) + 1), orgList));
						} else {
							sqlString.append(" keyword");
							sqlString.append(hashPos.get(j) + 1);
							sqlString.append("='");
							sqlString.append(mainKeyToValueMap.get(mainKeys[j].getPk_keyword()));
							sqlString.append("'");
						}
						sqlString.append(" and  ");
					}
					sqlString.append(" ver= " + ver);
					sqlString.append(" group by t2.keyword");
					sqlString.append(subGropVO.getIndexByKeywordPK(pk_dynkeyword) + 1);
					BaseDAO dao = new BaseDAO();

					Map<String, Map<String, UFDouble>> result = new HashMap<String, Map<String, UFDouble>>();
					result = (Map<String, Map<String, UFDouble>>) dao
							.executeQuery(sqlString.toString(),
									new ResultSetProcessor() {
										private static final long serialVersionUID = 1L;
										@Override
										public Object handleResultSet(ResultSet rs) throws SQLException {
					...
					for (Map.Entry<String, Map<String, UFDouble>> enters : result.entrySet()) {
						...
						//要返回的结果
						Map<String, UFDouble> resultvalue = resultMap.get(key);
						if (resultvalue == null) {
							resultMap.put(key, value);
						} else {
							for (Map.Entry<String, UFDouble> s : value.entrySet()) {
								if (resultvalue.containsKey(s.getKey())) {
									resultvalue.put(s.getKey(), resultvalue.get(s.getKey()).add(s.getValue()));
								}else {
									resultvalue.put(s.getKey(), s.getValue());
								}
							}
						}
					}
		...
		try {
			...
			for(int i = 0 ; i < subKeys.length ; i++){
				...
				if(keyword.equals(pk_dynkeyword)){//动态区关键字的时候
					...
					//Modified by sunzeg 2017.8.29 当动态区关键字超过1000时采用零时表_begin
					if(otherWords.size() > 1000){
						PubDataDMO tempDMO = new PubDataDMO();
						String[] otherWordsArray = otherWords.toArray(new String[0]);
						int keyIdx = subGropVO.getIndexByKeywordPK(pk_dynkeyword)+1;
						String inTempTable = tempDMO.createTempTable(otherWordsArray, "keyword" + keyIdx);
						buf.append(inTempTable);
						buf.append(" and ");
					}else{
						buf2.append("keyword");
						...
						buf.append(" and ");
						//Modified by sunzeg 2017.8.29 当动态区关键字超过1000时采用零时表_end
					}					
				}else{
					buf.append("keyword");
...
//		}
		...
		if (map.size() > 0) {
			...
			//4.3:生成合并数版本
			genUnionRepData(pk_report,pubdata,datavos, measureProjectVOs, debitmap,creditmap);
	...
	private void genUnionRepData(String pk_report,MeasurePubDataVO pubdata,MeasureDataVO[] datavos,
			...
			Map<String, UFDouble> debitmap, Map<String, UFDouble> creditmap) {
		try {
			for (MeasureDataVO dvo : datavos) {
	 ...
	 *
	 */
	static class PubDataDMO extends DataManageObjectIufo {
		public String createTempTable(String[] pks,String key) throws SQLException, DbException {
			
			// 临时表表名
			String strTable = genTempName();
			...
			try {
				...
				strTable = tmptab.createTempTable(con, strTable, colSQL, null);
				StringBuffer sb = null;
				StringBuffer bufSQL = new StringBuffer();
				bufSQL.append("insert into ").append(strTable) .append(" ("+key+") ").append(" values ").append("(?)");
				...
				for(int i = 0 ;i < pks.length ; i++){
					...
					stmt.setString(1, pk);
					stmt.addBatch();
					if (i == pks.length - 1 || iBatchCount++ == 100) {
						stmt.executeBatch();
						iBatchCount = 0;
						stmt.clearBatch();
					}
				...
				sql.append("select ").append(" "+key+" ").append(" from ").append(strTable).append(") ");
				return sql.toString();
			} 
			catch (Exception e){
			  e.getMessage();
	...
	private static String genTempName() {
    String flag = genTimeStmap();
    return new StringBuilder().append("tmp_").append(IDMaker.makeID(5, IDMaker.TYPE_ALL)).append("_").append(flag).toString();
  }
	
	private static String genTimeStmap(){
    Calendar calendar = Calendar.getInstance();
    SimpleDateFormat fmt = (SimpleDateFormat) new SimpleDateFormat(
        "HHmm");
