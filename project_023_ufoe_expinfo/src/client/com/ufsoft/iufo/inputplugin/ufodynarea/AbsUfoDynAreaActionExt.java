...
import com.ufida.zior.docking.view.DockingView;
	...
	public boolean isEnabled()
	{
		...
		if (tableSetting != null && (tableSetting.getCellsAuth() == null
				|| tableSetting.getCellsAuth().isWritable(anchorPos.getRow(),
						anchorPos.getColumn())))
		{
			boolean isDyn = isInDynArea(cellsModel, anchorPos.getRow(), anchorPos.getColumn());
			if (isDyn && context.getAttribute(IUfoContextKey.IS_HBBBDATA)!=null && context.getAttribute(IUfoContextKey.IS_HBBBDATA).toString().equals("true")) {
				// @edit by zhoushuang at 2015-11-13,上午9:56:37 如果有筛选条件 则按钮不可用
//				----------------------------start--------------------------------
				if( getAnaDesigner().getAnaReportModel() != null){
					CellsModel designCellsModel = getAnaDesigner().getAnaReportModel().getFormatModel();
					DynamicAreaModel dynamicAreaModel = DynamicAreaModel.getInstance(designCellsModel);
					ExtendAreaCell[] exCells = dynamicAreaModel.getDynAreaCells();
					ExtendAreaCell exCell = null;
					for(ExtendAreaCell exCelle : exCells){
						String extendAreaPK = exCelle.getExAreaPK();
						AreaPosition area = exCelle.getArea();
						CellPosition[] posArr = area.split();
						boolean found = false;
						for (CellPosition fmtPos : posArr) {
							KeyVO key = dynamicAreaModel.getKeywordModel().getDynAreaKeyVOByPos(extendAreaPK, fmtPos);
							if(key != null && KeyVO.DIC_CORP_PK.equals(key.getPk_keyword())) {
								exCell = exCelle;
								found = true;
								break;
							}
					...
					if (exCell != null && exCell.getAreaInfoSet().getValueFilter() != null) {
						return false;
					}
				}
//				-----------------------------end----------------------------------
	...
	public AbsAnaReportDesigner getAnaDesigner(){
		...
		if (this.getCurrentView() instanceof AbsAnaReportDesigner){
			designer = (AbsAnaReportDesigner) this.getCurrentView();			
		}
		if(designer == null){//如果还为空，需要从MainBoard中继续找，找显示在用户面前的designer
			 Viewer viewer = getMainboard().getCurrentDocumentView();
			 if(viewer != null && viewer instanceof AbsAnaReportDesigner){//需要手工激活，否则有些功能会有问题
					Dockable d = getMainboard().getDockingManager().getDockable(viewer.getId());
					getMainboard().getPerspectiveManager().getCurrentPerspective().active((DockingView) d);
