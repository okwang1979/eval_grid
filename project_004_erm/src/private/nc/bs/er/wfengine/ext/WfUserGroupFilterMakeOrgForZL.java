...
public class WfUserGroupFilterMakeOrgForZL implements IParticipantFilter {
	...
	public HashSet<String> filterUsers(ParticipantFilterContext pfc)
		...
		try{
			...
			Logger.error("new wf group start:->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			Logger.error("wf group is:"+wfGroupVO);
			if(pfc.getBillEntity()!=null){
				Logger.error("Bill class is:"+pfc.getBillEntity().getClass().getName());
			}
			...
			if(pfc.getBillEntity() instanceof nc.vo.arap.basebill.BaseAggVO){
				...
				billNo = billvo.getHeadVO().getBillno();
				...
				Logger.error("Bill type  is BaseAggVO pk_billOrg is:"+pk_billorg);
			}
			
			if(pfc.getBillEntity() instanceof JKBXVO){
			
				JKBXVO billvo = (JKBXVO) pfc.getBillEntity();
				billNo = billvo.getParentVO().getDjbh();
				pk_billorg = (String) billvo.getParentVO().getAttributeValue(JKBXHeaderVO.FYDWBM);
				
				Logger.error("Bill type  is JKBXVO pk_billOrg(1) is:"+pk_billorg);
				...
				Logger.error("Bill type  is JKBXVO pk_bxOrg is:"+pk_bxOrg);
				if(pk_bxOrg!=null&&pk_bxOrg.trim().length()>0){
					...
					if(dmOrg!=null&&parentVo!=null){
						if(dmOrg.getInnercode().startsWith(parentVo.getInnercode())){
							Logger.error("Bill type  is JKBXVO  is  jk org true");
							
							
							
							pk_billorg = pk_bxOrg;
							
							Logger.error("Bill type  is JKBXVO pk_billOrg(2) is:"+pk_billorg);
						}
					}
			...
			Logger.error("*********************Bill no is:"+billNo);
					...
					if(roleVO.getRole_code().endsWith("_dept")){
						for (String orgdeptid : roleVO.getAssignOrgPks()) {
							if(StringUtils.equals(pk_billorg, roleVO.getPk_org())
								...
								Logger.error("use role is:"+"("+roleVO.getRole_code()+")"+roleVO.getRole_name());
								break;
							}
						}
					}
...
//			没有找到，再找同单位的
			...
			Logger.error("Find use user pk_org is:"+task.getParticipantBelongOrg());
			...
			for(String userID : effectUserIDs) {
				Logger.error("Query user by role: "+userID);
				if(!userIDs.contains(userID)) {
					Logger.error("           be del: "+userID);
					effectUserIDs.remove(userID);
				}
			}
			Logger.error("new wf group end:-<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
