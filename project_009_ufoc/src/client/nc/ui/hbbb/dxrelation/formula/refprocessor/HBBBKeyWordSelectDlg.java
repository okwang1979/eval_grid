...
public class HBBBKeyWordSelectDlg  extends UfoDialog implements ActionListener{
	 ...
	 * @param funcInfo 
	 */
	public HBBBKeyWordSelectDlg(Container parent, ChooseKeyVO[] hasDefVos, UfoFuncInfo funcInfo) {
		super(parent);
		if (hasDefVos == null)
	 ...
	 * @param hasDefVos
	 */
	public HBBBKeyWordSelectDlg(Dialog parent, ChooseKeyVO[] hasDefVos) {
		super(parent);
		if (hasDefVos == null)
			vos = new ChooseKeyVO[0];
		else
...
//			mainPane.add(getKeyPane(), BorderLayout.NORTH);
	...
	private JTable getTable() {
		...
		IDxModelFunction func = NCLocator.getInstance().lookup(IDxModelFunction.class);
		List<ChooseKeyVO>  keyList = new ArrayList<>(); 
		try {
			List<Object[]> arrays  = (List)func.queryChooseKeyValue("select code ,name from iufo_keyword");
			for(Object[] strs:arrays){
				ChooseKeyVO key = new ChooseKeyVO();
				if(strs[0] !=null){

					key.setCode(String.valueOf(strs[0]));
				}
				key.setName1(String.valueOf(strs[1]));
				keyList.add(key);
			}
			...
			Logger.error(e1);
			throw new BusinessRuntimeException("查询关键字错误!");
		}
		tablemodel = new KeyWordTableModel(keyList.toArray(new ChooseKeyVO[0]));
