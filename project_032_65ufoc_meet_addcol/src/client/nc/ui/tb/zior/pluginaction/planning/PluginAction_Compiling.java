...
//				String code_unit = tbPlanContext.
			...
			//setActionCode为""，不能set为null lys 20150617
			tbPlanContext.setAction_Code(ITbPlanActionCode.code_Compile);
...
//				}
//			}
			//任务的创建系统是否为当前系统 --- 针对汇率维护，非当前系统创建的任务不可编辑
			if(!isBusiSystemValid(task, tbPlanContext))
				return ;
...
//			CostTime  ct = new CostTime();
			TBSheetViewer viewer = (TBSheetViewer)getCurrentView();
			ViewManager manager = viewer.getViewManager();
			manager.setVarValidChangedMap();
//			ct.printStepCost("setVarValidChangedMap cost--------->:");
...
//					}
//				}
				/**modified by pengzhen2015.3.6*/
				if (valueCale == -1 && TbCompliePlanConst.COM_MODE_TASKVIEW == status
						&&(tbPlanContext.getIsCurrAndAimcurrSame()||tbPlanContext.getNodeType().equals(ITbPlanActionCode.RATEMAINTENANCE_NODETYPE))
						&&(ITaskStatus.STARTED.equals(planstatus)
								|| ITaskStatus.ADJUSTING.equals(planstatus) ||
								ITaskStatus.COMPILING.equals(planstatus) ||
								ITaskStatus.APPROVE_NOTPASS.equals(planstatus) ||
								ITaskStatus.LOCAL_APPROVE_NOTPASS.equals(planstatus)
								)&&tbPlanContext.getTaskNumber()==1){
					return true;
				}
			}
		}
